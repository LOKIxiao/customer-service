...
一开始把测试都切到 `FakeMCPToolClient` 之后忘了给 RAG 检索也传 Fake embedding，结果单测意外命中了 `.env` 里配置的真实 Qwen embedding API，一条单测跑到 40+ 秒；后来知识库扩到 8 篇（82 个 chunk）后，冷启动跑全部真实 embedding 一次要 1 分钟左右。这个经历可以用来回答"你怎么保证测试速度和真实性两不误"——答案是显式传 Fake 依赖，而不是依赖全局配置/环境变量的默认值。
...
写了 30 条 QA 数据集（`data/eval/retrieval_qa.json`，覆盖全部 8 篇文档，混合直给关键词和换问法两种），跑 `python -m app.rag.evaluation` 对比三种配置的 recall@k 和 MRR：

| 配置 | Fake embedding | 真实 Qwen embedding |
| --- | --- | --- |
| naive（InMemory，纯向量） | recall@3 53.3%，MRR 0.472 | recall@3 100%，MRR 0.983 |
| chroma（纯向量） | recall@3 53.3%，MRR 0.472 | recall@3 100%，MRR 0.983 |
| chroma + BM25（混合） | recall@3 **76.7%**，MRR **0.650** | recall@3 100%，MRR 0.978 |

**这个结果比"混合检索总是更好"更有说服力**：embedding 弱的时候混合检索价值明显（53%→77%），但换成高质量的真实 embedding，纯向量已经接近满分，混合检索边际收益很小，甚至因为 BM25 引入排序噪声让 MRR 略微下降。这体现了"什么时候该用什么技术"的判断，而不是无脑上高级技巧。

## 长期记忆部分

### 为什么是"分层"，而不是把窗口开大点

原来的 `SessionMemory` 只是一个 session 内最近 10 条消息的滑动窗口，唯一用途是回答"你刚才问了什么"这一个专项问题，完全不参与正常回复生成——`ResponseAgent` 传给 LLM 的只有 `user_message`/`intent`/`raw_reply`，历史消息压根没进 Prompt。单纯把窗口开大解决不了根本问题：滑动窗口存的是原始对话，跨 session、跨很多轮之后依然会丢失（超过 max_messages 就被挤掉），而且原始对话里大部分内容是一次性的（查了个物流状态），并不是都值得长期记住。
...
1. **意图路由没跟上知识库扩充**：知识库扩到 8 篇文档，但 `HybridIntentAgent` 的意图分类还是订单/退款/工单/记忆几类，问会员等级、优惠券、故障排查这些问题会分类成 `unknown`，压根不会触发 RAG 检索。这是自己在做完检索优化后，手动测试时才发现的问题——好的意图识别覆盖不上不代表检索没做好，但确实说明"端到端测试"和"单元测试"要分开看，这是下一步要优先修的。
2. `ChromaVectorStore` 每次懒加载都整体删除重建 collection，没有增量同步；这个规模下（82 个 chunk）够用，规模变大需要改成基于内容 hash 的增量更新。
3. 首次检索会对全部 chunk 顺序调用 embedding 接口，没有批量请求，真实 embedding 下冷启动有几十秒延迟。
4. RRF 混合检索不是万能药，见上面评估结果的结论。
5. 没有真正的 cross-encoder rerank（成本收益考虑后用 RRF 代替）。
...
**Q: 你怎么评估 RAG 效果的？**
A: 写了 30 条人工标注的 QA 数据集，量化 recall@k 和 MRR，对比了 naive 向量检索、chroma 向量检索、混合检索三种配置，在 Fake 和真实 embedding 两种条件下都跑了一遍，发现效果和 embedding 质量强相关（详见上面的表格）。
...
A: 短期记忆（`SessionMemory`）是 session 内的滑动窗口，解决"这句话是不是在追问上一句"，天然有窗口大小限制、跨 session 就没了；长期记忆（`UserMemoryStore`）是跨 session、按 `user_id` 持久化的向量库，解决"这个用户一直以来有什么稳定偏好"。两者面向的时间尺度和存储的信息类型都不一样，只留一个要么丢失长期上下文，要么把大量一次性对话内容也硬存下来。
...
- 测试：70 个，~6 秒跑完（早期一度因为意外命中真实 embedding API 跑到 90 秒）
- 评估数据集：30 条 QA，覆盖全部 8 篇文档
- 混合检索相对纯向量检索提升（弱 embedding 场景）：recall@3 53.3% → 76.7%
